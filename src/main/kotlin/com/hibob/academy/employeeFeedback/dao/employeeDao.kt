...
class employeeDao(private val sql: DSLContext) {
    ...
    fun getEmployeeById(id: Int): EmployeeInfo {
        val employee = sql.select(employeeTable.id, employeeTable.firstName, employeeTable.lastName, employeeTable.role, employeeTable.department, employeeTable.companyId)
            .from(employeeTable)
            .where(employeeTable.id.eq(id))
            .fetch(employeeMapper)

        return employee ?: throwException<BadRequestException>{}
